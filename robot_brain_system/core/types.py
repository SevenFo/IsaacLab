...
class SkillInitError(Exception):
    """Custom exception for skill initialization errors."""
...
class BaseSkill:
    ...
    def __init__(self):
        ...
        full_cfg = load_config(config_path=str(config_path), return_dict=True)
    ...
    # NOTE: Unified interface: skills must implement `select_action(obs_dict)`
    # and return an `Action` object. Removed compute_action/get_status shims
    # to enforce a single canonical API.
...
class SkillPlan:
    ...
    def get_next_pending_skill_with_index(self) -> Optional[tuple[int, SkillStep]]:
        ...
        for i, step in enumerate(self.steps):
            if step.status == SkillStatus.PENDING or step.status == SkillStatus.FAILED:
...
class SystemState:
    ...
    # DEPRECATED: obs_history now managed by EnvProxy buffer (use env_proxy.get_and_clear_observation_buffer())
