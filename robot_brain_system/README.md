...
### 模型启动
1. 启动 qwen2.5-vl-32b-awq 
```bash
 CUDA_VISIBLE_DEVICES=0,2 vllm serve /quick_data/models/Qwen/Qwen2.5-VL-32B-Instruct-AWQ --tensor-parallel-size 2 --gpu-memory-utilization 0.7 --max-model-len 4096 
```

### 多模态输入
1. 如果采用 video 进行输入，视频往往会收到不同实现方式的抽帧进行预处理，因此更加推荐直接将视频帧作为图片输入，并自行实现抽帧逻辑。
...
## 编辑说明
```
env_isaacsim/lib/python3.10/site-packages/transformers/processing_utils.py
        ...
        if tokenize:
            ...
            for conversation in conversations:
                ...
                for message in conversation:
                    ...
                    for fname_idx, fname in enumerate(video_fnames):
                        if isinstance(fname, (list, tuple)) and (isinstance(fname[0], str) or isinstance(fname[0], Image.Image)):
                            video = [np.array(load_image(image_fname)) for image_fname in fname]
                            # create a 4D video because `load_video` always returns a 4D array
                            video = np.stack(video)
                            metadata = video_metadatas[fname_idx]
                            logger.warning(
