...
import numpy as np
import torch  # For type hinting if needed, though data is numpy
...
class LunarEnvRPyCService(rpyc.Service if RPYC_AVAILABLE else object):
            ...
            with self._lock:  # Ensure thread-safe creation
                if self._env is None:  # Double-check after acquiring lock
                    if LunarBaseEnv is None:
                        raise RuntimeError(
                            "LunarBaseEnv class not available. Check import errors."
                        )
                    ...
                    try:
                        ...
                        self._env = LunarBaseEnv(scene_cfg=scene_cfg, **env_args)
                        print(
    ...
    def _check_env_status(self):
        ...
        if self._env is None:  # Should not happen if _ensure_env_created works
            raise rpyc.core.vinegar.GenericException(
    ...
    def exposed_close_env_instance(self):
        ...
        with self._lock:
            if self._env and not self._env._is_closed:
                ...
                print("[RPyC Service] LunarBaseEnv instance was already closed.")
            else:
    ...
    def exposed_action_space_info(self) -> dict:
        ...
        return {
            ...
            "low": space.low.tolist(),  # Convert numpy to list for easier JSON/pickle
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="RPyC Server for LunarBaseEnv.")
    parser.add_argument(
