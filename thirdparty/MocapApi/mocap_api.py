...
class MCPAvatar(object):
  ...
  def get_avatar_posture_ptp_time(self):
    ...
    if err != MCPError.NoError:
      raise RuntimeError('Can not get avatar posture ptp time code: {0}'.format(MCPError._fields[err]))
...
class MCPAliceHub(object):
    ...
    class MCPAliceHubApi(Structure):
        _fields_ = [
            ...
            ('GetMarkerList', CFUNCTYPE(c_int32, POINTER(MCPMarkerHandle), POINTER(MCPAliceBusHandle))),
...
class MCPMarker(object):
    ...
    def get_marker_position(self):
      marker_x = marker_y = marker_z = c_float()
