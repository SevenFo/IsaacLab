...
def set_default_box_pose(
    env: ManagerBasedEnv,
    env_ids: torch.Tensor,
    default_pose: torch.Tensor,
    box_cfg: SceneEntityCfg = SceneEntityCfg("box"),
):
    # Set the default pose for box in all envs
    asset = env.scene[box_cfg.name]
    asset.write_root_pose_to_sim(torch.tensor(default_pose, device=env.device).repeat(env.num_envs, 1))
    
    
def set_default_desk_pose(
    env: ManagerBasedEnv,
    env_ids: torch.Tensor,
    default_pose: torch.Tensor,
    desk_cfg: SceneEntityCfg = SceneEntityCfg("desk"),
):
    # Set the default pose for desk in all envs
    asset = env.scene[desk_cfg.name]
    asset.data.default_box_pos = torch.tensor(default_pose, device=env.device).repeat(env.num_envs, 1)
...
def randomize_object_pose(
    ...
    # 如果只有一个箱子，直接生成随机位姿
    if len(asset_cfgs) == 1:
        asset_cfg = asset_cfgs[0]
        asset = env.scene[asset_cfg.name]

        for cur_env in env_ids.tolist():
            # 生成随机位姿
            pose = sample_object_poses(pose_range=pose_range)[0]  # 取出单个物体的位姿
