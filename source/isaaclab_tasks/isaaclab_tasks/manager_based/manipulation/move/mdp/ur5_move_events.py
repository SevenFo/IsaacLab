...
def randomize_object_pose(
    ...
    for cur_env in env_ids.tolist():
        ...
        # 添加小的扰动到asset2的x, y和yaw
        # 定义扰动范围（可以根据需要调整）
        pos_perturb_scale = 0.005  # 位置扰动尺度：5mm
        rot_perturb_scale = 0.07   # 旋转扰动尺度：约度
        
        # 生成随机扰动
        pos_perturb = torch.randn(1, 2, device=env.device) * pos_perturb_scale  # x, y方向扰动
        rot_perturb = torch.randn(1, 1, device=env.device) * rot_perturb_scale  # yaw方向扰动
        
        # 应用位置扰动到x, y坐标
        # positions2[:, 0] += pos_perturb[:, 0]  # x方向
        positions2[:, 1] += pos_perturb[:, 1]  # y方向
        
        # 应用旋转扰动（只影响yaw）
        perturb_quat = math_utils.quat_from_euler_xyz(
            torch.tensor([0.0], device=env.device),
            torch.tensor([0.0], device=env.device),
            rot_perturb[:, 0]
        )
        orientations2 = math_utils.quat_mul(orientations2, perturb_quat)
