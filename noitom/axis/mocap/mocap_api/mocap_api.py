...
MocapApi = cdll.LoadLibrary(
    os.path.join(
        os.path.dirname(__file__),
        {
            "DarWin": "",
            "Linux": "linux/x64/libMocapApi.so",
...
class MCPRigidBody(object):
    ...
    class MCPRigidBodyApi(Structure):
        _fields_ = [
            ...
            (
                "GetRigidBodyStatus",
                CFUNCTYPE(c_int32, POINTER(c_int32), MCPRigidBodyHandle),
            ),
            (
                "GetRigidBodyId",
                CFUNCTYPE(c_int32, POINTER(c_int32), MCPRigidBodyHandle),
    ...
    def __init__(self, rigid_body_handle):
        if not self.api:
            err = MocapApi.MCPGetGenericInterface(
                self.IMCPRigidBodyApi_Version, pointer(self.api)
            )
            if err != MCPError.NoError:
                raise RuntimeError(
                    "Can not get MCPSensorModule interface: {0}".format(
                        MCPError._fields[err]
                    )
                )
    ...
    def get_rotation(self):
        ...
        z = c_float()
        w = c_float()
        err = self.api.contents.GetRigidBodyRotation(
            pointer(x), pointer(y), pointer(z), pointer(w), self.handle
        )
        if err != MCPError.NoError:
            raise RuntimeError(
                "Can not get rigid body rotation: {0}".format(MCPError._fields[err])
            )
    ...
    def get_position(self):
        ...
        y = c_float()
        z = c_float()
        err = self.api.contents.GetRigidBodyPosition(
            pointer(x), pointer(y), pointer(z), self.handle
        )
        if err != MCPError.NoError:
            raise RuntimeError(
                "Can not get rigid body position: {0}".format(MCPError._fields[err])
            )
    ...
    def get_status(self):
        ...
        if err != MCPError.NoError:
            raise RuntimeError(
                "Can not get rigid body status: {0}".format(MCPError._fields[err])
            )
        return status.value
...
class MCPSensorModule(object):
    ...
    def __init__(self, sensor_handle):
        if not self.api:
            err = MocapApi.MCPGetGenericInterface(
                self.IMCPSensorModuleApi_Version, pointer(self.api)
            )
            if err != MCPError.NoError:
                raise RuntimeError(
                    "Can not get MCPSensorModule interface: {0}".format(
    ...
    def get_posture(self):
        ...
        z = c_float()
        w = c_float()
        err = self.api.contents.GetSensorModulePosture(
            pointer(x), pointer(y), pointer(z), pointer(w), self.handle
        )
        if err != MCPError.NoError:
            raise RuntimeError(
                "Can not get sensor module posture: {0}".format(MCPError._fields[err])
            )
        return w.value, x.value, y.value, z.value
    ...
    def get_angular_velocity(self):
        ...
        y = c_float()
        z = c_float()
        err = self.api.contents.GetSensorModuleAngularVelocity(
            pointer(x), pointer(y), pointer(z), self.handle
        )
        if err != MCPError.NoError:
            raise RuntimeError(
                "Can not get sensor module angular velocity: {0}".format(
                    MCPError._fields[err]
                )
            )
        return x.value, y.value, z.value
    ...
    def get_accelerated_velocity(self):
        ...
        y = c_float()
        z = c_float()
        err = self.api.contents.GetSensorModuleAcceleratedVelocity(
            pointer(x), pointer(y), pointer(z), self.handle
        )
        if err != MCPError.NoError:
            raise RuntimeError(
                "Can not get sensor module accelerated velocity: {0}".format(
                    MCPError._fields[err]
                )
            )
        return x.value, y.value, z.value
...
class MCPBodyPart(object):
    ...
    def __init__(self, body_part_handle):
        if not self.api:
            err = MocapApi.MCPGetGenericInterface(
                self.IMCPBodyPartApi_Version, pointer(self.api)
            )
            if err != MCPError.NoError:
                raise RuntimeError(
                    "Can not get MCPBodyPartApi interface: {0}".format(
                        MCPError._fields[err]
                    )
                )
    ...
    def get_position(self):
        ...
        y = c_float()
        z = c_float()
        err = self.api.contents.GetJointPosition(
            pointer(x), pointer(y), pointer(z), self.handle
        )
        if err != MCPError.NoError:
            raise RuntimeError(
                "Can not get joint position:{0}".format(MCPError._fileds[err])
            )
        return x.value, y.value, z.value
    ...
    def get_displacement_speed(self):
        ...
        y = c_float()
        z = c_float()
        err = self.api.contents.GetJointDisplacementSpeed(
            pointer(x), pointer(y), pointer(z), self.handle
        )
        if err != MCPError.NoError:
            raise RuntimeError(
                "Can not get joint displacement speed: {0}".format(
                    MCPError._fields[err]
                )
            )
        return x.value, y.value, z.value
    ...
    def get_posture(self):
        ...
        z = c_float()
        w = c_float()
        err = self.api.contents.GetBodyPartPosture(
            pointer(x), pointer(y), pointer(z), pointer(w), self.handle
        )
        if err != MCPError.NoError:
            raise RuntimeError(
                "Can not get body part posture: {0}".format(MCPError._fields[err])
            )
        return w.value, x.value, y.value, z.value
...
class MCPJoint(object):
    ...
    class MCPJointApi(Structure):
        _fields_ = [
            ...
            (
                "GetJointChild",
                CFUNCTYPE(
                    c_int32, POINTER(MCPJointHandle), POINTER(c_uint32), MCPJointHandle
                ),
            ),
            (
                "GetJointBodyPart",
                CFUNCTYPE(c_int32, POINTER(MCPBodyPartHandle), MCPJointHandle),
            ),
            (
                "GetJointSensorModule",
                CFUNCTYPE(c_int32, POINTER(MCPSensorModuleHandle), MCPJointHandle),
            ),
            ("GetJointTag", CFUNCTYPE(c_int32, POINTER(c_int32), MCPJointHandle)),
            ("GetJointNameByTag", CFUNCTYPE(c_int32, POINTER(c_char_p), c_int32)),
            (
                "GetJointChildJointTag",
                CFUNCTYPE(c_int32, POINTER(c_int32), POINTER(c_uint32), c_int32),
            ),
    ...
    def __init__(self, joint_handle):
        if not self.api:
            err = MocapApi.MCPGetGenericInterface(
                self.IMCPJointApi_Version, pointer(self.api)
            )
            if err != MCPError.NoError:
                raise RuntimeError(
                    "Can not get MCPJointApi interface: {0}".format(
                        MCPError._fields[err]
                    )
                )
    ...
    def get_name(self):
        ...
        if err != MCPError.NoError:
            raise RuntimeError(
                "Can not get joint name: {0}".format(MCPError._fields[err])
            )
    ...
    def get_local_rotation(self):
        ...
        z = c_float()
        w = c_float()
        err = self.api.contents.GetJointLocalRotation(
            pointer(x), pointer(y), pointer(z), pointer(w), self.handle
        )
        if err != MCPError.NoError:
            raise RuntimeError(
                "Can not get joint local rotation: {0}".format(MCPError._fields[err])
            )
        return w.value, x.value, y.value, z.value
    ...
    def get_local_rotation_by_euler(self):
        ...
        y = c_float()
        z = c_float()
        err = self.api.contents.GetJointLocalRotationByEuler(
            pointer(x), pointer(y), pointer(z), self.handle
        )
        if err != MCPError.NoError:
            raise RuntimeError(
                "Can not get joint euler: {0}".format(MCPError._fields[err])
            )
        return x.value, y.value, z.value
    ...
    def get_local_position(self):
        ...
        y = c_float()
        z = c_float()
        err = self.api.contents.GetJointLocalTransformation(
            pointer(x), pointer(y), pointer(z), self.handle
        )
        if err != MCPError.NoError:
            return None
    ...
    def get_default_local_position(self):
        ...
        y = c_float()
        z = c_float()
        err = self.api.contents.GetJointDefaultLocalTransformation(
            pointer(x), pointer(y), pointer(z), self.handle
        )
        if err != MCPError.NoError:
            raise RuntimeError(
                "Can not get joint default local transformation: {0}".format(
                    MCPError._fields[err]
                )
            )
        return x.value, y.value, z.value
    ...
    def get_children(self):
        ...
        err = self.api.contents.GetJointChild(
            POINTER(MCPJointHandle)(), pointer(joint_count), self.handle
        )
        if err != MCPError.NoError:
            raise RuntimeError(
                "Can not get joint child count: {0}".format(MCPError._fields[err])
            )
        joint_handles = (MCPJointHandle * joint_count.value)()
        err = self.api.contents.GetJointChild(
            joint_handles, pointer(joint_count), self.handle
        )
        if err != MCPError.NoError:
            raise RuntimeError(
                "Can not get joint child: {0}".format(MCPError._fields[err])
            )
    ...
    def get_body_part(self):
        ...
        if err != MCPError.NoError:
            raise RuntimeError(
                "Can not get joint body part: {0}".format(MCPError._fields[err])
            )
        return MCPBodyPart(body_part_handle)

    def get_sensor_module(self):
        sensor_handle = MCPSensorModuleHandle()
        err = self.api.contents.GetJointSensorModule(
            pointer(sensor_handle), self.handle
        )
        if err != MCPError.NoError:
    ...
    def get_tag(self):
        ...
        if err != MCPError.NoError:
            raise RuntimeError(
                "Can not get joint tag: {0}".format(MCPError._fields[err])
            )
        return tag.value
    ...
    def get_name_by_tag(self, tag):
        ...
        if err != MCPError.NoError:
            raise RuntimeError(
                "Can not get joint name of joint tag {0}: {1}".format(tag, err)
            )
        return str(joint_name.value, encoding="utf8")

    def get_child_joint_tag(self, tag):
        joint_count = c_uint32()
        err = self.api.contents.GetJointChildJointTag(
            POINTER(c_int32)(), pointer(joint_count), c_int32(tag)
        )
        if err != MCPError.NoError:
            raise RuntimeError(
                "Can not get joint child joint tag: {0}".format(MCPError._fields[err])
            )
        joints_tag = (c_int32 * joint_count.value)()
        err = self.api.contents.GetJointChildJointTag(
            joints_tag, pointer(joint_count), c_int32(tag)
        )
        if err != MCPError.NoError:
            raise RuntimeError(
                "Can not get joint child joint tag: {0}".format(MCPError._fields[err])
    ...
    def get_parent_joint_tag(self, tag):
        ...
        if err != MCPError.NoError:
            raise RuntimeError(
                "can not get joint parent tag: {0}".format(MCPError._fields[err])
            )
...
class MCPAvatar(object):
    ...
    class MCPAvatarApi(Structure):
        _fields_ = [
            ...
            (
                "GetAvatarRootJoint",
                CFUNCTYPE(c_int32, POINTER(MCPJointHandle), MCPAvatarHandle),
            ),
            (
                "GetAvatarJoints",
                CFUNCTYPE(
                    c_int32, POINTER(MCPJointHandle), POINTER(c_uint32), MCPAvatarHandle
                ),
            ),
            (
                "GetAvatarJointByName",
                CFUNCTYPE(c_int32, c_char_p, POINTER(MCPJointHandle), MCPAvatarHandle),
            ),
            ...
            (
                ...
                CFUNCTYPE(c_int32, POINTER(c_uint32), POINTER(MCPAvatarHandle)),
            ),
            (
    ...
    def __init__(self, avatar_handle):
        if not self.api:
            err = MocapApi.MCPGetGenericInterface(
                self.IMCPAvatarApi_Version, pointer(self.api)
            )
            if err != MCPError.NoError:
                raise RuntimeError(
                    "Can not get MCPAvatar interface: {0}".format(MCPError._fields[err])
                )
    ...
    def get_index(self):
        ...
        if err != MCPError.NoError:
            raise RuntimeError(
                "Can not get avatar index: {0}".format(MCPError._fields[err])
            )
        return index.value
    ...
    def get_joints(self):
        joint_count = c_uint32()
        err = self.api.contents.GetAvatarJoints(
            POINTER(MCPJointHandle)(), pointer(joint_count), self.handle
        )
        if err != MCPError.NoError:
            raise RuntimeError(
                "Can not get avatar joints: {0}".format(MCPError._fields[err])
            )
        joints_handle = (MCPJointHandle * joint_count.value)()
        err = self.api.contents.GetAvatarJoints(
            joints_handle, pointer(joint_count), self.handle
        )
        if err != MCPError.NoError:
            raise RuntimeError(
                "Can not get avatar Joints: {0}".format(MCPError._fields[err])
    ...
    def get_joint_by_name(self, name):
        ...
        joint_handle = MCPJointHandle()
        err = self.api.contents.GetAvatarJointByName(
            joint_name, pointer(joint_handle), self.handle
        )
        if err != MCPError.NoError:
            raise RuntimeError(
                "Can not get avatar Joints: {0}".format(MCPError._fields[err])
    ...
    def get_name(self):
        ...
        if err != MCPError.NoError:
            raise RuntimeError(
                "Can not get avatar name: {0}".format(MCPError._fields[err])
            )
        return str(avatar_name.value, encoding="utf8")

    def get_rigid_bodies(self):
        rigid_body_count = c_uint32()
        err = self.api.contents.GetAvatarRigidBodies(
            POINTER(MCPRigidBodyHandle)(), pointer(rigid_body_count), self.handle
        )
        if err != MCPError.NoError:
            raise RuntimeError(
                "Can not get avatar rigid bodies: {0}".format(MCPError._fields[err])
            )
        rigid_body_handles = (MCPRigidBodyHandle * rigid_body_count.value)()
        err = self.api.contents.GetAvatarRigidBodies(
            rigid_body_handles, pointer(rigid_body_count), self.handle
        )
        if err != MCPError.NoError:
            raise RuntimeError(
                "Can not get avatar rigid bodies: {0}".format(MCPError._fields[err])
        ...
        return [
            MCPRigidBody(rigid_body_handles[i]) for i in range(rigid_body_count.value)
        ]
...
MCPEventType = namedtuple(
    "EMCPEventType", ["InvalidEvent", "AvatarUpdated", "RigidBodyUpdated", "Error"]
)(0, 256, 512, 768)
...
MCPBvhRotation = namedtuple(
    "EMCPBvhRotation", ["XYZ", "XZY", "YXZ", "YZX", "ZXY", "ZYX"]
)(0, 1, 2, 3, 4, 5)

MCPBvhData = namedtuple(
    "EMCPBvhData",
    ["String", "BinaryWithOldFrameHeader", "Binary", "BinaryLegacyHumanHierarchy"],
)(0, 1, 2, 6)

MCPBvhDisplacement = namedtuple("EMCPBvhDisplacement", ["Disable", "Enable"])(0, 1)
...
class MCPSettings(object):
    ...
    class MCPSettingsApi(Structure):
        _fields_ = [
            ...
            (
                "SetSettingsTCP",
                CFUNCTYPE(c_int32, c_char_p, c_uint16, MCPSettingsHandle),
            ),
            ("SetSettingsBvhRotation", CFUNCTYPE(c_int32, c_int32, MCPSettingsHandle)),
            (
                "SetSettingsBvhDisplacement",
                CFUNCTYPE(c_int32, c_int32, MCPSettingsHandle),
            ),
    ...
    def __init__(self):
        if not self.api:
            err = MocapApi.MCPGetGenericInterface(
                self.IMCPSettingsApi_Version, pointer(self.api)
            )
            if err != MCPError.NoError:
                raise RuntimeError(
                    "Can not get MCPSettings interface: {0}".format(
                        MCPError._fields[err]
                    )
                )
        ...
        if err != MCPError.NoError:
            raise RuntimeError(
                "Can not create settings: {0}".format(MCPError._fields[err])
            )
    ...
    def __del__(self):
        ...
        if err != MCPError.NoError:
            raise RuntimeError(
                "Can not destroy settings: {0}".format(MCPError._fields[err])
            )
    ...
    def set_udp(self, local_port):
        ...
        if err != MCPError.NoError:
            raise RuntimeError(
                "Can not set udp port of {0}: {1}".format(
                    local_port, MCPError._fields[err]
                )
            )

    def set_tcp(self, ip, port):
        err = self.api.contents.SetSettingsTCP(
            c_char_p(bytes(ip, encoding="utf8")), c_uint16(port), self.handle
        )
        if err != MCPError.NoError:
            raise RuntimeError(
                "Can not settings tcp addr of {0}:{1}: {2}".format(
                    ip, port, MCPError._fields[err]
                )
            )

    def set_bvh_rotation(self, bvh_rotation):
        err = self.api.contents.SetSettingsBvhRotation(
            c_int32(bvh_rotation), self.handle
        )
        if err != MCPError.NoError:
            raise RuntimeError(
                "Can not set bvh rotation: {0}".format(MCPError._fields[err])
            )

    def set_bvh_displacement(self, bvh_transformation):
        err = self.api.contents.SetSettingsBvhDisplacement(
            c_int32(bvh_transformation), self.handle
        )
        if err != MCPError.NoError:
            raise RuntimeError(
                "Can not set bvh displacement: {0}".format(MCPError._fields[err])
            )
    ...
    def set_bvh_data(self, bvh_data):
        ...
        if err != MCPError.NoError:
            raise RuntimeError(
                "Can not set bvh data: {0}".format(MCPError._fields[err])
            )
    ...
    def set_calc_data(self):
        ...
        if err != MCPError.NoError:
            raise RuntimeError(
                "Can not set calc data: {0}".format(MCPError._fields[err])
            )


MCPUpVector = namedtuple("EMCPUpVector", ["XAxis", "YAxis", "ZAxis"])(1, 2, 3)

MCPFrontVector = namedtuple("EMCPFrontVector", ["ParityEven", "ParityOdd"])(1, 2)

MCPCoordSystem = namedtuple("EMCPCoordSystem", ["RightHanded", "LeftHanded"])(0, 1)

MCPRotatingDirection = namedtuple(
    "EMCPRotatingDirection", ["Clockwise", "CounterClockwise"]
)(0, 1)

MCPPreDefinedRenderSettings = namedtuple(
    "EMCPPreDefinedRenderSettings", ["Default", "UnrealEngine", "Unity3D", "Count"]
)(0, 1, 2, 3)

MCPUnit = namedtuple("EMCPUnit", ["Centimeter", "Meter"])(0, 1)
...
class MCPRenderSettings(object):
    ...
    class MCPRenderSettingsApi(Structure):
        _fields_ = [
            (
                ...
                CFUNCTYPE(c_int32, POINTER(MCPRenderSettingsHandle)),
            ),
            (
                "GetPreDefRenderSettings",
                CFUNCTYPE(c_int32, c_int32, POINTER(MCPRenderSettingsHandle)),
            ),
            (
                "SetUpVector",
                CFUNCTYPE(c_int32, c_int32, c_int32, MCPRenderSettingsHandle),
            ),
            (
                "GetUpVector",
                CFUNCTYPE(
                    c_int32, POINTER(c_int32), POINTER(c_int32), MCPRenderSettingsHandle
                ),
            ),
            (
                "SetFrontVector",
            ...
            (
                "GetCoordSystem",
                CFUNCTYPE(c_int32, POINTER(c_int32), MCPRenderSettingsHandle),
            ),
            (
                "SetRotatingDirection",
                CFUNCTYPE(c_int32, c_int32, MCPRenderSettingsHandle),
            ),
            (
    ...
    def __init__(self, pre_def=None):
        if not self.api:
            err = MocapApi.MCPGetGenericInterface(
                self.IMCPRenderSettingsApi_Version, pointer(self.api)
            )
            if err != MCPError.NoError:
                raise RuntimeError(
                    "Can not get MCPRenderSettings interface: {0}".format(
                        MCPError._fields[err]
                    )
                )
        ...
        if self.pre_def == None:
            ...
            if err != MCPError.NoError:
                raise RuntimeError(
                    "Can not create render settings: {0}".format(MCPError._fields[err])
                )
        else:
            err = self.api.contents.GetPreDefRenderSettings(
                c_int32(pre_def), pointer(self.handle)
            )
            if err != MCPError.NoError:
                raise RuntimeError(
                    "Can not get render settings: {0}".format(MCPError._fields[err])
                )
    ...
    def __del__(self):
        if self.pre_def == None:
            ...
            if err != MCPError.NoError:
                raise RuntimeError(
                    "Can not destroy render settings: {0}".format(MCPError._fields[err])
                )

    def set_up_vector(self, up_vector, sign):
        err = self.api.contents.SetUpVector(
            c_int32(up_vector), c_int32(sign), self.handle
        )
        if err != MCPError.NoError:
            raise RuntimeError(
                "Can not set up vector: {0}".format(MCPError._fields[err])
            )

    def get_up_vector(self):
        up_vector = c_int32()
        sign = c_int32()
        err = self.api.contents.GetUpVector(
            pointer(up_vector), pointer(sign), self.handle
        )
        if err != MCPError.NoError:
            raise RuntimeError(
                "Can not get up vector: {0}".format(MCPError._fields[err])
            )
        return up_vector.value, sign.value

    def set_front_vector(self, front_vector, sign):
        err = self.api.contents.SetFrontVector(
            c_int32(front_vector), c_int32(sign), self.handle
        )
        if err != MCPError.NoError:
            raise RuntimeError(
                "Can not set front vector: {0}".format(MCPError._fields[err])
            )

    def get_front_vector(self):
        front_vector = c_int32()
        sign = c_int32()
        err = self.api.contents.GetFrontVector(
            pointer(front_vector), pointer(sign), self.handle
        )
        if err != MCPError.NoError:
            raise RuntimeError(
                "Can not get front vector: {0}".format(MCPError._fields[err])
            )
    ...
    def set_coord_system(self, coord_sys):
        ...
        if err != MCPError.NoError:
            raise RuntimeError(
                "Can not set coord system: {0}".format(MCPError._fields[err])
            )
    ...
    def get_coord_system(self):
        ...
        if err != MCPError.NoError:
            raise RuntimeError(
                "Can not get coord system: {0}".format(MCPError._fields[err])
            )
        return coord_sys.value

    def set_rotating_direction(self, rotating_direction):
        err = self.api.contents.SetRotatingDirection(
            c_int32(rotating_direction), self.handle
        )
        if err != MCPError.NoError:
            raise RuntimeError(
                "Can not set rotating direction: {0}".format(MCPError._fields[err])
            )

    def get_rotating_direction(self):
        rotating_direction = c_int32()
        err = self.api.contents.GetRotatingDirection(
            pointer(rotating_direction), self.handle
        )
        if err != MCPError.NoError:
            raise RuntimeError(
                "Can not get rotating direction: {0}".format(MCPError._fields[err])
            )
...
class MCPApplication(object):
    ...
    class MCPApplicationApi(Structure):
        _fields_ = [
            ...
            (
                ...
                CFUNCTYPE(c_int32, MCPSettingsHandle, MCPApplicationHandle),
            ),
            (
                "SetApplicationRenderSettings",
                CFUNCTYPE(c_int32, MCPRenderSettingsHandle, MCPApplicationHandle),
            ),
            ...
            (
                ...
                CFUNCTYPE(
                    c_int32, POINTER(c_uint64), POINTER(c_uint32), MCPApplicationHandle
                ),
            ),
            (
            ...
            (
                ...
                CFUNCTYPE(
                    c_int32, POINTER(MCPEvent), POINTER(c_uint32), MCPApplicationHandle
                ),
            ),
        ]
    ...
    def __init__(self):
        if not self.api:
            err = MocapApi.MCPGetGenericInterface(
                self.IMCPApplicationApi_Version, pointer(self.api)
            )
            if err != MCPError.NoError:
                raise RuntimeError(
                    "Can not get IMCPApplication interface: {0}".format(
                        MCPError._fields[err]
                    )
                )
        ...
        if err != MCPError.NoError:
            raise RuntimeError(
                "Can not create application: {0}".format(MCPError._fields[err])
            )
    ...
    def __del__(self):
        ...
        if err != MCPError.NoError:
            raise RuntimeError(
                "Can not destroy application: {0}".format(MCPError._fields[err])
            )
    ...
    def set_settings(self, settings):
        ...
        if err != MCPError.NoError:
            raise RuntimeError(
                "Can not set application settings: {0}".format(MCPError._fields[err])
            )

    def set_render_settings(self, settings):
        err = self.api.contents.SetApplicationRenderSettings(
            settings.handle, self._handle
        )
        if err != MCPError.NoError:
            raise RuntimeError(
                "Can not set application render settings: {0}".format(
                    MCPError._fields[err]
                )
            )
    ...
    def is_event_cache_enabled(self):
        enable = c_bool()
        err = self.api.contents.ApplicationCacheEventsIsEnabled(
            pointer(enable), self._handle
        )
        if err != MCPError.NoError:
            raise RuntimeError(
                "Can not get application event cache settings: {0}".format(
                    MCPError._fields[err]
                )
            )
        return enable.value
    ...
    def get_rigid_bodies(self):
        ...
        err = self.api.contents.GetApplicationRigidBodies(
            POINTER(MCPRigidBodyHandle)(), pointer(rigid_body_size), self._handle
        )
        if err != MCPError.NoError:
            raise RuntimeError(
                "Can not get application rigid bodies: {0}".format(
                    MCPError._fields[err]
                )
            )
        rigid_body_handles = (MCPRigidBodyHandle * rigid_body_size.value)()
        err = self.api.GetApplicationRigidBodies(
            rigid_body_handles, pointer(rigid_body_size), self._handle
        )
        if err != MCPError.NoError:
            raise RuntimeError(
                "Can not get application rigid bodies: {0}".format(
        ...
        return [
            MCPRigidBody(rigid_body_handles[i]) for i in range(rigid_body_size.value)
        ]

    def get_avatars(self):
        avatar_count = c_uint32()
        err = self.api.contents.GetApplicationAvatars(
            POINTER(MCPAvatarHandle)(), pointer(avatar_count), self._handle
        )
        if err != MCPError.NoError:
            raise RuntimeError(
                "Can not get application avatars: {0}".format(MCPError._fields[err])
            )
        avatar_handles = (MCPAvatarHandle * avatar_count.value)()
        err = self.api.contents.GetApplicationAvatars(
            avatar_handles, pointer(avatar_count), self._handle
        )
        if err != MCPError.NoError:
            raise RuntimeError(
                "Can not get application avatars: {0}".format(MCPError._fields[err])
    ...
    def poll_next_event(self):
        ...
        if err != MCPError.NoError:
            raise RuntimeError(
                "Can not poll application next event: {0}".format(MCPError._fields[err])
            )
